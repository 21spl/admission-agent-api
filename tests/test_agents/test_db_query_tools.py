...
@pytest.mark.asyncio
async def test_application_query_tools_status_tool_returns_error_dict_for_missing(db_session):
    ...
    status_tool = next(t for t in tools if t.metadata.name == "get_student_application_status")
    result = await status_tool.async_fn()
    assert result == {"error": "No application found."}


@pytest.mark.asyncio
async def test_application_query_tools_status_tool_returns_real_data(db_session, test_application):
    from app.ai.tools.application_query_tools import build_application_query_tools
...
@pytest.mark.asyncio
async def test_document_query_tools_pending_documents_lists_all_required_when_none_uploaded(db_session, test_application):
    ...
    pending_tool = next(t for t in tools if t.metadata.name == "get_pending_or_missing_documents")
...
@pytest.mark.asyncio
async def test_offer_query_tools_no_offers_returns_empty_list(db_session, test_application):
    ...
    offers_tool = next(t for t in tools if t.metadata.name == "get_student_admission_offers")
    result = await offers_tool.async_fn()
    assert result == []


@pytest.mark.asyncio
async def test_loan_query_tools_no_loan_returns_error_dict(db_session, test_application):
