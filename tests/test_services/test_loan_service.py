...
from app.models.enums import AllowedFileType, ApplicationStatus, LoanStatus
...
def _mock_llm_with_income(income_value):
    ...
    fake_llm = MagicMock()
    text = (
        f'{{"annual_income_inr": {income_value}}}'
        if income_value is not None
        else '{"annual_income_inr": null}'
    )
...
async def test_request_loan_raises_404_when_no_application(db_session, test_student):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await loan_service.request_loan(
            test_student, "income.pdf", b"fake", AllowedFileType.PDF
        )
    assert exc_info.value.status_code == 404


@pytest.mark.asyncio
async def test_request_loan_raises_409_when_offer_not_accepted(
...
@pytest.mark.asyncio
async def test_request_loan_raises_409_when_decision_already_exists(
    db_session, test_student, test_application
):
    ...
    with (
        ...
        with pytest.raises(HTTPException) as exc_info:
            await loan_service.request_loan(
                test_student, "income2.pdf", b"fake2", AllowedFileType.PDF
            )
...
@pytest.mark.asyncio
async def test_request_loan_raises_422_when_income_extraction_fails(
    db_session, test_student, test_application
):
    ...
    with (
        ...
        patch(
            "app.services.loan_service.storage_manager.upload_document",
...
@pytest.mark.asyncio
async def test_request_loan_raises_422_when_pdf_has_no_readable_text(
    db_session, test_student, test_application
):
    await _set_status(db_session, test_application, ApplicationStatus.OFFER_ACCEPTED)
    loan_service = get_loan_service(db_session)
...
@pytest.mark.asyncio
async def test_request_loan_approves_when_income_below_threshold(
    db_session, test_student, test_application
):
    await _set_status(db_session, test_application, ApplicationStatus.OFFER_ACCEPTED)
    loan_service = get_loan_service(db_session)
    ...
    with (
        ...
        patch.object(
            loan_service,
            "_read_pdf_text",
            return_value=f"Annual income: Rs. {below_threshold}",
        ),
        _mock_storage_patches(),
    ):
...
@pytest.mark.asyncio
async def test_request_loan_rejects_when_income_above_threshold(
    db_session, test_student, test_application
):
    await _set_status(db_session, test_application, ApplicationStatus.OFFER_ACCEPTED)
    loan_service = get_loan_service(db_session)
    ...
    with (
        ...
        patch.object(
            loan_service,
            "_read_pdf_text",
            return_value=f"Annual income: Rs. {above_threshold}",
        ),
        _mock_storage_patches(),
    ):
        loan = await loan_service.request_loan(
...
@pytest.mark.asyncio
async def test_request_loan_income_exactly_at_threshold_is_approved(
    db_session, test_student, test_application
):
    ...
    with (
        ...
        patch.object(
            loan_service,
            "_read_pdf_text",
            return_value=f"Annual income: Rs. {exact_threshold}",
        ),
        _mock_storage_patches(),
    ):
        loan = await loan_service.request_loan(
...
@pytest.mark.asyncio
async def test_request_loan_raises_422_on_malformed_llm_json(
    db_session, test_student, test_application
):
    ...
    with (
        ...
        patch.object(loan_service, "_read_pdf_text", return_value="Some document text"),
    ):
        with pytest.raises(HTTPException) as exc_info:
            await loan_service.request_loan(
                test_student, "income.pdf", b"fake", AllowedFileType.PDF
...
@pytest.mark.asyncio
async def test_request_loan_raises_502_on_storage_failure(
    db_session, test_student, test_application
):
...
@pytest.mark.asyncio
async def test_get_loan_application_raises_404_when_no_application(
...
@pytest.mark.asyncio
async def test_get_loan_application_raises_404_when_no_loan_yet(
    db_session, test_student, test_application
):
    loan_service = get_loan_service(db_session)
    with pytest.raises(HTTPException) as exc_info:
...
@pytest.mark.asyncio
async def test_get_loan_application_returns_real_loan(
    db_session, test_student, test_application
):
    await _set_status(db_session, test_application, ApplicationStatus.OFFER_ACCEPTED)
    loan_service = get_loan_service(db_session)
    ...
    with (
        ...
        created = await loan_service.request_loan(
            test_student, "income.pdf", b"fake", AllowedFileType.PDF
        )
...
@pytest.mark.asyncio
async def test_get_loan_application_by_application_id_returns_none_when_absent(
    ...
    """
    loan_service = get_loan_service(db_session)
    result = await loan_service.get_loan_application_by_application_id(
        test_application.id
    )
