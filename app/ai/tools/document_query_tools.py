...
def build_document_query_tools(db, application_id) -> List[FunctionTool]:
    ...
    async def get_document_download_link(document_type: DocumentType) -> dict:
        ...
        doc_ = await document_service.get_document_by_application_id_and_type(application_id, document_type)
        if doc_ is None:
            return {"error": "No document found."}
        download_link = await document_service.get_download_link(doc_.id)
        return {"document_id": doc_.id, "document_type": doc_.doc_type, "download_link": download_link}
    ...
    return [
        FunctionTool.from_defaults(async_fn=list_my_documents, name="list_student_uploaded_documents",
            description="Lists all documents uploaded for the logged-in student's application, with validation status."),
        FunctionTool.from_defaults(async_fn=document_upload_pending, name="get_pending_or_missing_documents",
            description="Returns required document types the student has NOT uploaded yet."),
        FunctionTool.from_defaults(async_fn=get_document_download_link, name="get_document_download_link",
        ...
        FunctionTool.from_defaults(async_fn=inspect_validation_issue, name="inspect_application_validation_issues",
