...
from app.storage import StorageFetchError, StorageUploadError, storage_manager
...
class DocumentService:
    ...
    async def upload_document_metadata(
        ...
        storage_key = storage_manager.build_student_doc_key(
            application.id, doc_type.value, filename
        )
        ...
        if existing_doc:
            ...
            await self.application_service.update_application_status(
                ...
                f"STUDENT_OVERWRITE_{student.id}",
            )
            return updated_doc
    ...
    async def get_all_document_bytes_for_application(
        self, application_id: uuid.UUID
    ...
    async def check_all_document_types_uploaded(
        self, application_id: uuid.UUID
    ) -> bool:
        ...
        if required_types.issubset(uploaded_types):
            await self.application_service.update_application_status(
                application_id, ApplicationStatus.ALL_DOCS_UPLOADED, "AI"
            )
            return True
        return False

    async def mark_auto_validated(
        self, application_id: uuid.UUID, doc_types: list[str]
    ) -> None:
        application = await self.application_repository.get_by_id(application_id)
        if not application:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Application entry not found.",
            )
    ...
    async def mark_auto_rejected(
        self, application_id: uuid.UUID, reason: str, doc_types: list[str]
    ) -> None:
        application = await self.application_repository.get_by_id(application_id)
        if not application:
            raise HTTPException(
