...
async def list_all_branches(service: BranchService = Depends(get_branch_service)):
    """
...
@router.get(
    "/{branch_id}", response_model=BranchResponse, status_code=status.HTTP_200_OK
)
async def get_single_branch(
    branch_id: uuid.UUID, service: BranchService = Depends(get_branch_service)
):
    """
