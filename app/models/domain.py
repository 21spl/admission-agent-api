...
class Student(Base):
    ...
    marks_physics: Mapped[float | None] = mapped_column(Float, nullable=True)
    marks_chemistry: Mapped[float | None] = mapped_column(Float, nullable=True)
    marks_maths: Mapped[float | None] = mapped_column(Float, nullable=True)
    marks_english: Mapped[float | None] = mapped_column(Float, nullable=True)
    marks_computer_science: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class Branch(Base):
    ...
    cutoff_marks: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class Application(Base):
    ...
    validation_flags: Mapped[int | None] = mapped_column(Integer, nullable=True)
    validation_issues: Mapped[str | None] = mapped_column(String, nullable=True)
    ...
    loan_application: Mapped[Optional["LoanApplication"]] = relationship(
        "LoanApplication", back_populates="application", cascade="all, delete-orphan"
    )
...
class ShortlistingPreference(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "application_id", "preference_order", name="uq_application_pref_order"
        ),
...
class Document(Base):
    ...
    validation_reason: Mapped[str | None] = mapped_column(String(500), nullable=True)
...
class ApplicationStatusHistory(Base):
    ...
    )  # e.g., 'SYSTEM' or Officer UUID string
    ...
    application: Mapped["Application"] = relationship(
        "Application", back_populates="history"
    )
...
class LoanApplication(Base):
    ...
    income_certificate_doc_id: Mapped[uuid.UUID] = mapped_column(
    ...
    extracted_annual_income: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    application: Mapped["Application"] = relationship(
        "Application", back_populates="loan_application"
    )
...
class Offer(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "application_id", "round_number", name="uq_offer_application_round"
        ),
    )
